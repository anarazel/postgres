...
#
# This plht_test has two serializable transactions. Both select two rows
...
setup
{
  CREATE TABLE plht_test (i int PRIMARY KEY, t text);
  INSERT INTO plht_test VALUES (5, 'apple'), (7, 'pear'), (11, 'banana');
  -- HOT-update 'pear' row.
  UPDATE plht_test SET t = 'pear_hot_updated' WHERE i = 7;
}

teardown
{
  DROP TABLE plht_test;
}

session s1
step b1 { BEGIN ISOLATION LEVEL SERIALIZABLE; }
step r1 { SELECT * FROM plht_test WHERE i IN (5, 7) }
step w1 { UPDATE plht_test SET t = 'pear_xact1' WHERE i = 7 }
step c1 { COMMIT; }

session s2
step b2 { BEGIN ISOLATION LEVEL SERIALIZABLE; }
step r2 { SELECT * FROM plht_test WHERE i IN (5, 7) }
step w2 { UPDATE plht_test SET t = 'apple_xact2' WHERE i = 5 }
step c2 { COMMIT; }
