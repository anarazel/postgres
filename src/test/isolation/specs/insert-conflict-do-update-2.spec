...
setup
{
  CREATE TABLE icdu2_upsert (key text not null, payload text);
  CREATE UNIQUE INDEX ON icdu2_upsert(lower(key)) INCLUDE (payload);
}

teardown
{
  DROP TABLE icdu2_upsert;
}
...
step insert1 { INSERT INTO icdu2_upsert(key, payload) VALUES('FooFoo', 'insert1') ON CONFLICT (lower(key)) DO UPDATE set key = EXCLUDED.key, payload = icdu2_upsert.payload || ' updated by insert1'; }
step c1 { COMMIT; }
step a1 { ABORT; }
...
step insert2 { INSERT INTO icdu2_upsert(key, payload) VALUES('FOOFOO', 'insert2') ON CONFLICT (lower(key)) DO UPDATE set key = EXCLUDED.key, payload = icdu2_upsert.payload || ' updated by insert2'; }
step select2 { SELECT * FROM icdu2_upsert; }
step c2 { COMMIT; }
