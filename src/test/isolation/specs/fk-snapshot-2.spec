...
setup
{
  CREATE TABLE fs2_parent (parent_id SERIAL NOT NULL PRIMARY KEY);
  CREATE TABLE fs2_child (
	child_id SERIAL NOT NULL PRIMARY KEY,
	parent_id INTEGER REFERENCES fs2_parent);
  INSERT INTO fs2_parent VALUES(1);
}

teardown { DROP TABLE fs2_parent, fs2_child; }
...
step s1del	{ DELETE FROM fs2_parent WHERE parent_id = 1; }
step s1c	{ COMMIT; }
step s1ins	{ INSERT INTO fs2_parent VALUES (2); }
...
step s2ins	{ INSERT INTO fs2_child VALUES (1, 1); }
step s2c	{ COMMIT; }
step s2sel	{ SELECT count(*) FROM fs2_parent; }
step s2ins2	{ INSERT INTO fs2_child VALUES (2, 2); }
...
# A fs2_parent row committed by another transaction after this one took its
...
# correctly not visible: referencing it is a violation.  Only the fs2_parent-side
...
# and the fs2_parent row is visible.
