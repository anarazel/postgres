# Read-write-unique rwu3_test.
# From bug report 9301.

setup
{
  CREATE TABLE rwu3_test (
    key   integer UNIQUE,
    val   text
  );
  ...
  LANGUAGE SQL AS $$
    INSERT INTO rwu3_test (key, val) SELECT k, v WHERE NOT EXISTS (SELECT key FROM rwu3_test WHERE key = k);
  $$;
}
