...
setup
{
 CREATE TABLE ct_a (i int);
 CREATE FUNCTION ct_f() RETURNS TRIGGER LANGUAGE plpgsql AS 'BEGIN RETURN NULL; END;';
 INSERT INTO ct_a VALUES (0), (1), (2), (3);
}
...
session s1
step s1a { BEGIN; }
step s1b { CREATE TRIGGER t AFTER UPDATE ON ct_a EXECUTE PROCEDURE ct_f(); }
step s1c { COMMIT; }

session s2
step s2a { BEGIN; }
step s2b { SELECT * FROM ct_a WHERE i = 1 FOR UPDATE; }
step s2c { UPDATE ct_a SET i = 4 WHERE i = 3; }
step s2d { COMMIT; }
