...
CREATE FUNCTION sqljson_stable_one() RETURNS int AS $$ BEGIN RETURN 1; END; $$ LANGUAGE plpgsql STABLE;
...
EXPLAIN (VERBOSE, COSTS OFF) SELECT JSON_OBJECT('a': JSON_OBJECTAGG('b': sqljson_stable_one() RETURNING text) FORMAT JSON);
SELECT JSON_OBJECT('a': JSON_OBJECTAGG('b': sqljson_stable_one() RETURNING text) FORMAT JSON);
...
DROP FUNCTION volatile_one, sqljson_stable_one;
