...
main(int argc, char **argv)
{
	...
	/*
	 * pg_waldump can emit a large volume of output on stdout (>= one line per
	 * WAL record using several writes per record). On windows this can end up
	 * being extremely slow due to the small default buffer sizes. To avoid
	 * that, we use full buffering, with explicit fflush() calls in
	 * appropriate places.
	 *
	...
	config.isatty = isatty(fileno(stdout));
