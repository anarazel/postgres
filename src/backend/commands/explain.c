			...
			ExplainState *es)
{
	...
	switch (nodeTag(plan))
	{
		case T_IndexScan:
			...
			{
				show_buffer_usage(es, &((IndexScanState *) planstate)->iss_Instrument.table_bufusage, "Table");

				if (planstate->instrument)
				{
					BufferUsage zero = {0};

					BufferUsageAccumDiff(&planstate->instrument->bufusage, &zero,
										 &((IndexScanState *) planstate)->iss_Instrument.table_bufusage);
				}
			}
	...
	if (es->workers_state && (es->buffers || es->wal) && es->verbose)
	{
		...
		for (int n = 0; n < w->num_workers; n++)
		{
			...
			ExplainOpenWorker(n, es);
			if (es->buffers)
				show_buffer_usage(es, &instrument->bufusage, NULL);
			if (es->wal)
...
show_indexsearches_info(PlanState *planstate, ExplainState *es)
{
		...
		case T_IndexScan:
			{
				...
				table_misses = indexstate->iss_Instrument.table_misses;
	...
	ExplainPropertyUInteger("Table Fetch Misses", NULL, table_misses, es);
}
