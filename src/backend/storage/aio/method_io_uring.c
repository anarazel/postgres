...
pgaio_uring_check_capabilities(void)
{
...
#if defined(HAVE_LIBURING_QUEUE_INIT_MEM) && defined(IORING_SETUP_NO_MMAP) && 1
	{
		...
		if (ring_ptr == MAP_FAILED)
			elog(ERROR, "mmap(%zu) to determine io_uring_queue_init_mem() support has failed: %m",
				 ring_size);
		...
		if (ret > 0)
		{
			...
			elog(DEBUG1, "can use combined memory mapping for io_uring, each ring needs %d bytes", ret);
		...
		else
		{
			 ...
			 */
			errno = -ret;
			elog(DEBUG1, "cannot use combined memory mapping for io_uring, ring creation failed with %m");
...
#else
	{
		elog(DEBUG1, "can't use combined memory mapping for io_uring, kernel or liburing too old");
	}
#endif
